...
duration = st.sidebar.slider("Duration (s)", 1.0, 10.0, 4.0, 0.5)
fs = st.sidebar.slider("Sampling Frequency", 100, 2000, 1000, 100)

st.sidebar.subheader("Amplitude Envelope")
amp_freq = st.sidebar.slider("Envelope frequency (Hz)", 0.1, 5.0, 1.2, 0.1)

st.sidebar.subheader("Frequency Modulation")
base_freq = st.sidebar.slider("Base frequency (Hz)", 5.0, 50.0, 20.0, 1.0)
mod_depth = st.sidebar.slider("Modulation depth", 0.0, 20.0, 6.0, 0.5)
mod_freq = st.sidebar.slider("Modulation frequency (Hz)", 0.1, 10.0, 0.8, 0.1)
